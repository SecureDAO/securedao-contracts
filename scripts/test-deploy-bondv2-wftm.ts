...
async function main() {
    ...
    const chainlinkBondingCalculator = await new ChainlinkBondingCalculatorDeployer(deployer, scr.address).deploy();
    ...
    await treasury.connect(signer).queue('4', bond.address);
    await treasury.connect(signer).toggle('4', bond.address, chainlinkBondingCalculator.address);

    await treasury.connect(signer).queue('5', principleAddress);
    await treasury.connect(signer).toggle('5', principleAddress, chainlinkBondingCalculator.address);

    await treasury.isLiquidityToken(principleAddress).then((addr:string)=>console.log("is liquidity token ", addr))
    await treasury.bondCalculator(principleAddress).then((addr:string)=>console.log("bond calculator for lp ", addr))
    console.log("chainlink calc ", chainlinkBondingCalculator.address)

    console.log("bond deployed to ", bond.address);
    await bond.bondPriceInUSD().then((price:BigNumber)=> console.log("bond price USD ", price));
}
